  ...
  useEffect(() => {
    ...
    const checkVehicle = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/driver/check-vehicle-no?vehicle_no=${encodeURIComponent(vehicleValue)}`);
        const data = await response.json();
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">National ID (NID) Number</label>
                  <input
                    type="text"
                    {...register('nid_no', { 
                    ...
                    className={`w-full px-3 py-2 border rounded-md bg-white/50 focus:bg-white/70 
                    ${errors.nid_no ? 'border-red-500' : 'border-gray-300 focus:ring-2 focus:ring-blue-500/50'} 
                    shadow-sm focus:shadow-blue-100 transition-all`}
                  />
                  {errors.nid_no && <p className="mt-1 text-sm text-red-600">{errors.nid_no.message}</p>}
                </div>
