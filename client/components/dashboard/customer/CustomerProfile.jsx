...
import { FaUser, FaPhone, FaEnvelope, FaMapMarkerAlt, FaEdit, FaSave, FaTimes, FaCreditCard, FaSpinner, FaMoneyBillWave } from 'react-icons/fa';
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          payment_methods: response.data.payment_methods || ['cash']
        });
      } else {
        ...
        if (initialUserData) {
          setFormData({
            ...
            payment_methods: initialUserData.payment_methods || ['cash']
          });
        }
      }
    } catch (error) {
    ...
    setFormData((prev) => {
      ...
      if (currentMethods.includes(method)) {
        ...
        if (newMethods.length === 0) {
          ...
          toast.warning('At least one payment method must be selected');
          return prev;
        }
      } else {
  ...
  const handleCancel = () => {
    ...
    if (profileData) {
      setFormData({
        ...
        sex: profileData.sex || '',
        payment_methods: profileData.payment_methods || ['cash']
      });
    }
    setIsEditing(false);
  };
    ...
    <div>
      ...
      <Card className="p-6 mb-6 border border-gray-200 bg-white shadow-sm">
        ...
        <div className="space-y-3">
          {isEditing ? (
            <div className="space-y-3">
              <p className="text-sm text-gray-600 mb-3">Select your preferred payment methods:</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {['cash', 'bkash'].map((method) => (
                  <label 
                    key={method} 
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2">
                      {method === 'cash' && <FaMoneyBillWave className="text-green-600" />}
                      {method === 'bkash' && <span className="text-pink-500 font-bold text-sm">bK</span>}
            ...
            <div className="space-y-3">
              {(profileData?.payment_methods || ['cash']).map((method) => (
                <div key={method} className="flex items-center justify-between p-3 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border border-gray-200">
                  <div className="flex items-center space-x-3">
                    {method === 'cash' && <FaMoneyBillWave className="text-green-600" />}
                    {method === 'bkash' && <span className="text-pink-500 font-bold">bKash</span>}
                    {method === 'card' && <FaCreditCard className="text-blue-600" />}
                    <span className="text-gray-900 font-medium capitalize">{method} Payment</span>
                  </div>
                  {method === 'cash' && (
                    <span className="px-2 py-1 bg-green-100 text-green-800 rounded text-sm font-medium">Default</span>
                  )}
                  {method === 'bkash' && (
                    <span className="px-2 py-1 bg-pink-100 text-pink-800 rounded text-sm font-medium">Active</span>
                  )}
                  {method === 'card' && (
                    <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-sm font-medium">Linked</span>
                  )}
                </div>
              ))}
              <div className="text-center pt-3">
                <p className="text-sm text-gray-600">
                  Click "Edit Profile" to manage your payment methods
                </p>
              </div>
            </div>
          )}
